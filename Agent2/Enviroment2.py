...
class CustomEnv(gym.Env, Tetramino):
  ...
  def get_state(self):
      ...
      for row in range(24):
          if row == 20:
              for i in range(4):
                  top_4.append(self.observation_space[row+i])
              break

          if 1 in self.observation_space[row]:
              for i in range(4):
                  top_4.append(self.observation_space[row+i])
              break

      return self.current_tet, top_4
  ...
  def step(self, action):
    ...
    return (self.get_state()), reward, self.playing, info
